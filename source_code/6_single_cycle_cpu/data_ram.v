...
//   >         同步写，异步读，新增低电平同步复位，复位清空全部内存
//   > 作者  : LOONGSON
//   > 日期  : 2016-04-14
//*************************************************************************
module data_ram(
    input         clk,         // 时钟
    input         resetn,     // 新增低电平同步复位
    input  [3:0]  wen,        // 字节写使能
    input  [4:0]  addr,       // 地址
    ...
    reg [31:0] DM[31:0];  //数据存储器，32个32bit字单元
    integer i; // 把循环变量i提到模块顶层，不在always内部定义
    ...
    begin
        if (!resetn) begin
            ...
            if (wen[3]) DM[addr][31:24] <= wdata[31:24];
            if (wen[2]) DM[addr][23:16] <= wdata[23:16];
            if (wen[1]) DM[addr][15: 8] <= wdata[15: 8];
            if (wen[0]) DM[addr][7 : 0] <= wdata[7 : 0];
        end
    end
