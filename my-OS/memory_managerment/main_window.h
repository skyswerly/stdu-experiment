...
public:
    explicit MainWindow(QWidget* parent = nullptr);//explicit关键字修饰单参数构造函数避免隐式类型转换
    ~MainWindow() override;

//私有槽函数
private slots:
    //【应用参数】按钮：校验输入、重新生成地址、刷新地址表、重置输出
    void onApplyParams();

    //算法单选按钮切换：更换 replacer、重置页表和输出
    void onAlgorithmChanged();

    //【开始模拟】按钮：逐条执行 20 条地址访问，刷新输出表格和统计
    void onStartSimulation();

    //【清空输出表】按钮：清空输出表格、重置页表、清零统计
    void onClearTable();
...
private:
    ...
    //算法操作按钮控件
    QRadioButton* m_fifoRadio;    // FIFO算法
    QRadioButton* m_lruRadio;     // LRU算法
    QPushButton*  m_startBtn;     // 开始模拟
    QPushButton*  m_clearBtn;     // 清空输出表
    QPushButton*  m_exitBtn;      // 退出程序

    //指令地址顺序展示标签(20行)
    QTableWidget* m_addressTable;  // 逻辑地址 + 页号

    //页表状态标签
    QTableWidget* m_outputTable;   //20行对应20条指令执行后页表的状态

    //统计标签
    QLabel* m_hitCountLabel;   // 命中次数标签
    QLabel* m_hitRateLabel;    // 命中率标签
};
