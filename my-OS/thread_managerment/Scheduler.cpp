...
void Scheduler::advanceToNextArrival()
{
    ...
    {
        cout << "⏩ 时间推进到 " << minArrival << "ms（等待进程到达）" << endl;
...
void Scheduler::schedule()
{
    ...
    cout << "时间片: " << timeSlice << "ms" << endl << endl;

    // 按到达时间排序
    sort(readyQueue.begin(), readyQueue.end(), [](MyThread* a, MyThread* b)
    {
    ...
    while (finishProcesses < totalProcesses)
    {
        ...
        cout << "----------------------------------------" << endl;
        cout << "时间 " << currentTime << "ms: 选择进程 "
             << selected->getName() << " (优先级: " << selected->getPriority() << ")" << endl;
        ...
        if (isPreemptive())
        {
            ...
            if (selected->getRemainTime() <= 0)
            {
                ...
                finishProcesses++;
                cout << "✅ 进程 " << selected->getName() << " 已完成！" << endl;
                readyQueue.erase(
                    remove(readyQueue.begin(), readyQueue.end(), selected),
                    readyQueue.end());
            }
            else
            {
...
void Scheduler::displayResults()
{
    ...
    {
        cout << "\n========== 平均指标 ==========" << endl;
        cout << "平均周转时间: " << fixed << setprecision(2)
             << totalTurnaround / count << "ms" << endl;
        cout << "平均带权周转时间: " << fixed << setprecision(2)
             << totalWeighted / count << endl;
    }
}
